...
export default function Page(){
  ...
  const[form,setForm]=useState({title:'',event_date:today,event_time:'',audience:'KELOMPOK',activity_type_id:'',level_id:'',class_id:'',notes:''});
  ...
  useEffect(()=>{
    Promise.all([
      ...
      setActivities(a);setLevels(l);setClasses(c);setRole(u.role??'');
      if(u.role==='DEWAN_GURU')setForm(v=>({...v,audience:'MUDA_MUDI'}));
    });
  },[]);
  ...
  useEffect(()=>{
    ...
    fetch('/api/participants?'+params).then(r=>r.json()).then((p:Participant[])=>{
      setPeople(p);setStatus(Object.fromEntries(p.map(x=>[x.id,'H'])));
    });
  },[form.audience,form.level_id,form.class_id]);
  ...
  const allowed=useMemo(()=>audiences.filter(([v])=>role==='DEWAN_GURU'?['MUDA_MUDI','CABERAWIT'].includes(v):role==='KELOMPOK'?['KELOMPOK','MUDA_MUDI','IBU_IBU','PENGURUS'].includes(v):true),[role]);
    ...
    <form onSubmit={save}>
      <section className="card">
        <div className="formGrid">
          ...
          <label>Kategori<select className="select" value={form.audience} onChange={e=>setForm({...form,audience:e.target.value,activity_type_id:'',level_id:'',class_id:''})}>{allowed.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label>
          {(form.audience==='CABERAWIT'||form.audience==='MUDA_MUDI')&&<label>Kelas<select className="select" value={form.class_id} onChange={e=>setForm({...form,class_id:e.target.value})}><option value="">Semua kelas</option>{classOptions.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label>}
          {form.audience==='CABERAWIT'&&<label>Jenjang<select className="select" value={form.level_id} onChange={e=>setForm({...form,level_id:e.target.value})}><option value="">Semua jenjang</option>{levels.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select></label>}
          ...
          <label>Jenis kegiatan<select className="select" value={form.activity_type_id} onChange={e=>setForm({...form,activity_type_id:e.target.value})}><option value="">Pilih</option>{matching.map(a=><option value={a.id} key={a.id}>{a.name}</option>)}</select></label>
        </div>
      </section>
      <section className="section">
        ...
        <div className="list">
          ...
          {!people.length&&<div className="emptyState">Belum ada peserta pada kategori/kelas ini.</div>}
        </div>
      </section>
